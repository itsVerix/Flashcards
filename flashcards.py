...
class Logger:
    ...
    def save_logs(self):
        ...
        with open(logger_file, "w", encoding="utf-8") as log_file:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--import_from", type=str,
                        help="If --import_from=IMPORT is passed, where IMPORT is the file name, read the initial card "
                             "set from the external file, and print the message n cards have been loaded.")
    parser.add_argument("--export_to", type=str,
                        help="If --export_to=EXPORT is passed, where EXPORT is the file name, write all cards that "
                             "are in the program memory into this file after the user has entered exit, and the last "
                             "line of the output should be n cards have been saved.")
